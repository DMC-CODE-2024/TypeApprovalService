...
public class Process {
    ...
    private static void updateTrcStatusInNwl(Connection conn) {
        // String yesterDayDate = LocalDate.now().minusDays(1L).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        String formattedDate = QueryExecuter.getLastRunDate(conn);
        String t = "UPDATE app.national_whitelist nw SET nw.trc_imei_status = 1,   nw.trc_modified_time = CURRENT_TIMESTAMP WHERE nw.trc_imei_status IN (0, 3)  AND EXISTS (  SELECT 1  FROM app.mobile_device_repository mdr   WHERE mdr.device_id = nw.tac AND mdr.is_type_approved = '1'  AND mdr.trc_approval_date >= '" + formattedDate + "'   ) ";
